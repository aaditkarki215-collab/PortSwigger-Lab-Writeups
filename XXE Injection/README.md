...
A practical reference for identifying, exploiting, and preventing XXE vulnerabilities — from basic file disclosure to blind out-of-band exfiltration.
...
- **Perform SSRF** — pivot the server into making requests to internal-only systems
...
It's a long-standing member of the OWASP Top 10, and it keeps showing up because the root cause isn't usually bad application code — it's an **XML parser shipping with unsafe defaults**.
...
When `&xxe;` is referenced, the parser fetches and inlines the **contents of the referenced resource** — a local file, or even a URL. This single feature is the foundation of every technique below.

### Parameter entities

A special entity type prefixed with `%`, usable only inside the DTD itself. Parameter entities become important later because, unlike regular entities, they **can be joined when referenced from an external DTD** — which is the trick that unlocks CDATA-based and error-based exfiltration.
...
- **Content-Type coercion** — some apps default to JSON but will still happily parse XML if you change the header and reformat the body. Always try flipping `application/json` → `application/xml` on a POST request, even if the app "only uses JSON"
...
```
Swap the reflected field's content for `&company;`. If the response shows `test value` instead of the literal string `&company;`, the app is resolving entities → vulnerable.
...
Referencing a source file directly (`file:///var/www/html/index.php`) usually **fails silently** — PHP source contains `<`, `>`, `&`, which break XML parsing, and the entity reference is discarded.
...
If the entity's value is reflected back, you get full two-way interaction with the internal target (e.g., cloud metadata endpoints, internal admin panels). If not, it's blind SSRF — still dangerous, but you're limited to inferring success via timing/side effects. See the Server-Side Attacks / SSRF material for full exploitation technique — it applies identically once you can reach an internal URL this way.
...
4. **If output breaks** on special/binary content → CDATA + parameter entity method
5. **If nothing is reflected at all** → check for verbose errors → error-based method
6. **If there's no output AND no errors** → out-of-band exfiltration (HTTP or DNS)
